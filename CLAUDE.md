...
`app/globals.css` carries the reconciled palette, in three tiers: primitives (`--g-*` — the only place a colour literal belongs), semantic roles borrowed from the installed `tokens/colors.json` framework, and the shadcn bridge those roles feed. Three design-system colours were darkened to pass WCAG AA and the reasons are recorded inline. Two roles that used to share one token are now separate: **`--primary` is the action colour (signal) and `--destructive` is failure and deletion** — a filled primary button and an error banner are no longer the same red. Map onto a semantic token rather than pasting hex out of the prototype, and raise a palette change rather than making it from a feature branch (P7).
